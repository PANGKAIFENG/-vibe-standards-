---
description: 强制执行测试驱动开发工作流。先搭建接口，首先生成测试，然后实现最少代码以通过。确保 80%+ 覆盖率。
---
...
1. **搭建接口** - 首先定义类型/接口
2. **首先生成测试** - 编写失败的测试（RED 红灯）
3. **实现最少代码** - 仅编写足够通过测试的代码（GREEN 绿灯）
4. **重构** - 在保持测试绿灯的同时改进代码（REFACTOR 重构）
5. **验证覆盖率** - 确保 80%+ 测试覆盖率
...
```
用户: /tdd 我需要一个函数来计算市场流动性得分

代理 (tdd-guide):
# TDD 会话：市场流动性得分计算器

## 步骤 1：定义接口（SCAFFOLD 搭建）
